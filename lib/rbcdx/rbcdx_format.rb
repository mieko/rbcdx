  ...
  module RbcdxFormat
    ...
    FileMetadata = Struct.new(:path, :header, :header_length, :file_size, :sections) do
      def first_section_offset
        RbcdxFormat::MAGIC.bytesize + 4 + header_length
      end
      ...
      def read_section(name)
        ...
        File.open(path, "rb") do |file|
          ...
          unless data&.bytesize == bounds.length
            raise Error, "#{path}: truncated rbcdx #{RbcdxFormat.section_label(name)}"
          end

          data
        end
      rescue SystemCallError => error
        raise Error, "#{path}: invalid rbcdx #{RbcdxFormat.section_label(name)} section bounds: #{error.message}"
      end
    end
